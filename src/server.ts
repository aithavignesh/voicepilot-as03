...
async function answerFor(text:string){ const parsed=intentFor(text); const c=await context(); let answer=fallback(c,parsed.intent); let source='grounded-fallback'; if(openai){ const completion=await openai.chat.completions.create({model:process.env.OPENAI_MODEL||'gpt-4.1-mini',temperature:.2,messages:[{role:'system',content:'You are VoicePilot, a voice-first engineering copilot. Use only supplied project context. Never fabricate project facts. Be concise and practical. Distinguish facts from recommendations. This is a read-only public demo.'},{role:'user',content:JSON.stringify({request:text,intent:parsed,projectContext:c})}]}); answer=completion.choices[0]?.message?.content?.trim()||answer; source='openai'; } return {answer,intent:parsed.intent,entities:parsed.entities,source}; }
